...
- **紧凑核心指南：**把直接结论与真正有用的工具、平台、产品、人物、机构、方法或作品合并呈现，不机械拆章。
...
- **实操细节：**对实践型任务，在核心指南中明确所需工具和准备步骤，并给出第一次实操流程、决策规则与必要的安全或质量检查。
...
内容模型版本为 `0.3`。领域梳理、实操准备、知识依赖和来源验证仍是内部
研究检查项，但不会再机械生成重复的独立章节；旧 JSON 模型需要迁移后再
构建。
