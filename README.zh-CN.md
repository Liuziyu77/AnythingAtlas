...
- **主动问询：**AnythingAtlas 只追问会改变结果的信息，包括目标、当前水平、紧迫程度、单次可用时间、内容形式、语言、访问条件与期望深度。
- **先给答案：**先给直接结论、具体建议和关键取舍，不让用户先读一大段泛泛背景。
- **实战领域地图：**展示用户真正会遇到的工具、平台、产品、人物、机构、方法、作品或代表性参与者。
...
- **资源适配：**按短时速览、均衡学习或深度参考等路线组织资料，说明每条路线适合谁、按什么顺序看什么内容。
- **实操工具箱：**对实践型任务，明确所需工具和准备步骤，并给出第一次实操流程、决策规则与必要的安全或质量检查。
...
聚焦澄清并确认需求：AnythingAtlas 主动引导与提问，深入了解用户水平与需求
   ↓
直接结论与领域实战地图：回答真正的问题，展示具体选项
   ↓
实操工具箱：准备步骤、第一次实操与决策规则
   ↓
主题分类，以及资源发现、验证、排序与筛选
   ↓
适合用户节奏的资源路线与详细个性化学习计划
   ↓
来源与信息渠道附录：说明信息质量如何验证
   ↓
...
两份文件统一采用“先给答案”的顺序：需求简报、直接结论、领域实战
地图、实操工具箱、知识图谱、适合用户节奏的资源路线、精选资源、详细
路线图、来源策略、来源说明与下一步行动。

“先给答案”的内容模型版本为 `0.2`。它用必填的 `orientation`、
`field_guide`、`action_kit` 与 `resource_tracks` 取代了旧版
`topic_brief` 和 `starting_point`；旧 JSON 模型需要迁移后再构建。
